...
power_data$Global_reactive_power = kw(as.character(power_data$Global_reactive_power))
power_data$Voltage = kw(as.character(power_data$Voltage))
...
df1 <- data.frame(power_data$Global_reactive_power,power_data$Voltage,power_data$Global_active_power, power_data$Sub_metering_1, power_data$Sub_metering_2, power_data$Sub_metering_3, DateTime = as.POSIXct(paste(power_data$Date, power_data$Time)))
png(filename="Plot4.png", width=480, height=480, bg = "transparent")
par(mfrow=c(2,2))
plot(df1$DateTime, df1$power_data.Global_active_power, ylab="Global Active Power (kilowatts)", xlab="", type="l")
plot(df1$DateTime, df1$power_data.Voltage, ylab="Voltage", xlab="datetime", type="l")
...
plot(df1$DateTime, df1$power_data.Global_reactive_power, ylab="Global_reactive_power", xlab="datetime", type="l")
dev.off()
